...
function setupGuidance(features, selectedExercise = "general") {
  ...
  const required = config?.required || ["shoulders", "hips"];
  const checklist = [
  ...
  if (!messages.length) {
    messages.push(selectedExercise === "general" ? "Move into frame or select an exercise." : "Tracking is ready.");
  }
...
function candidateScores(features, history) {
  ...
  return {
    ...
    plank: round(plank, 3),
    situp: round(situp, 3),
    lateral_raise: round(lateralRaise, 3),
    deadlift: round(deadlift, 3),
    general: 0.18,
  };
}

function chooseExercise(scores, smoothedScores, currentExercise, manualExercise = "general") {
  ...
  EXERCISE_ORDER.forEach((exercise) => {
    nextScores[exercise] = round((nextScores[exercise] ?? 0) * 0.72 + (scores[exercise] ?? 0) * 0.28, 3);
  });
  ...
  if (!bestExercise || bestScore < 0.34) {
    return {
      exercise: "general",
  ...
  if (currentExercise !== "general" && currentScore >= bestScore - 0.08 && currentScore >= 0.36) {
    return {
  ...
  return {
    ...
    scores: nextScores,
    detectionStable: bestScore >= 0.42,
    lowConfidence: bestScore < 0.45,
  };
}
...
function updateRepCounter({ exercise, config, analysis, confidenceScore, progress, timestamp, repStates, totals, features }) {
  ...
  if (
    ...
    return { repEvent: invalidEvent, counted: false, invalid: true };
  }
  ...
  if (validCycle) {
    ...
    return { repEvent: validEvent, counted: true, invalid: false };
  }
...
function buildOffFrameState({ setup, totals, issueCounts, feedback, currentExercise, lastGoodState, selectedExercise }) {
  ...
  return {
    ...
    manualSelectionRecommended: activeExercise === "general",
    currentRepPhase: "ready",
    activeExercise,
...
function createAutoWorkoutTracker() {
  ...
  function update(keypoints = [], frame = { width: 1, height: 1 }, timestamp = Date.now(), options = {}) {
    ...
    const detection = chooseExercise(rawScores, smoothedScores, currentExercise, useAutoDetect ? "general" : selectedExercise);
      ...
      exerciseForAnalysis === "general"
        ? analysisResult(
            ...
            detection.lowConfidence
              ...
              : "Move into frame or select an exercise",
            [detection.lowConfidence ? "Auto-detect confidence is low. Select an exercise manually." : "Move into frame or select an exercise."],
            0,
            "unknown",
            {
              ...baseMetrics(features),
    ...
    const state = {
      ...
      trackingStable: true,
      manualSelectionRecommended: useAutoDetect && detection.lowConfidence,
      currentRepPhase:
        exerciseForAnalysis === "general" || exerciseForAnalysis === "plank"
          ? exerciseForAnalysis === "plank"
            ? "hold"
